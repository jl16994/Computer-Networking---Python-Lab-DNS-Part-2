...
def decrypt_with_aes(encrypted_data, password, salt):
    key = generate_aes_key(password, salt)
    f = Fernet(key)
    encrypted_data_bytes = eval(encrypted_data) # Convert str back to bytes
    decrypted_data = f.decrypt(encrypted_data_bytes) #call the Fernet decrypt method
    return decrypted_data.decode('utf-8')

salt = b'Tandon' # Remember it should be a byte-object
password = 'jl16994@nyu.edu'
        ...
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX: # For MX records
                    ...
                    txt_value = answer_data[0]
                    decrypted_txt_value = decrypt_with_aes(txt_value, password, salt)
                    # if decrypted_txt_value:
                        # print(f"Exfiltrated data: {decrypted_txt_value}")
                    rdata_list.append(dns.rdata.from_text(dns.rdataclass.IN, dns.rdatatype.TXT, f'"{txt_value}"'))
